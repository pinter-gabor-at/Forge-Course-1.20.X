...
import static net.kaupenjoe.mccourse.util.ModNameUtil.modBlockResourceLocation;
import static net.kaupenjoe.mccourse.util.ModNameUtil.modItemResourceLocation;
...
public class ModItemModelProvider extends ItemModelProvider {
	...
	protected void registerModels() {
		...
		complexBlock(ModBlocks.GEM_EMPOWERING_STATION);
	}

	@SuppressWarnings("SameParameterValue")
	private void complexBlock(RegistryObject<Block> block) {
		String path = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block.get())).getPath();
		withExistingParent(path, modBlockResourceLocation(path));
	}

	public void fenceItem(RegistryObject<Block> block, RegistryObject<Block> baseBlock) {
		withExistingParent(ModNameUtil.getPath(block), mcLoc("block/fence_inventory"))
			.texture("texture", modBlockResourceLocation(ModNameUtil.getPath(baseBlock)));
	}

	public void wallItem(RegistryObject<Block> block, RegistryObject<Block> baseBlock) {
		withExistingParent(ModNameUtil.getPath(block), mcLoc("block/wall_inventory"))
			.texture("wall", modBlockResourceLocation(ModNameUtil.getPath(baseBlock)));
	}
	...
	private ItemModelBuilder handheldItem(RegistryObject<Item> item) {
		return withExistingParent(item.getId().getPath(),
			...
			.texture("layer0", modItemResourceLocation(item.getId().getPath()));
	}
